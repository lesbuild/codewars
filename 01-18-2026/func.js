function countGrade(scores){
  let rank = {S:0, A:0, B:0, C:0, D:0, X:0}
  rank.D = scores.filter(a=>a >= 0 && a < 60).length;
  rank.C = scores.filter(a=>a >= 60 && a < 80).length;
  rank.B = scores.filter(a=>a >= 80 && a < 90).length;
  rank.A = scores.filter(a=>a >= 90 && a < 100).length;
  rank.S = scores.filter(a=>a ===100).length;
  rank.X = scores.filter(a=>a < 0).length;
  return rank
}
// Description:
