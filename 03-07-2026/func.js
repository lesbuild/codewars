function howMuchCoffee(events) {
  let count = 0
  let coffeeEvents = ["cw","movie","dog","cat","CW","MOVIE","DOG","CAT"]
  for(let i = 0;i<events.length;i++){
    if(coffeeEvents.includes(events[i]) && events[i] == events[i].toUpperCase()){
      count += 2
    }else if(coffeeEvents.includes(events[i])){
      count++
    }
  }
